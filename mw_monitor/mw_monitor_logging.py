...
def serialize_calls():
    ...
    try:
        with open(mwmon.api_tracer_bin_log_name, "w") as f_out:
...
def serialize_interproc():
    from mw_monitor_classes import mwmon
    try:
        with open(mwmon.interproc_bin_log_name, "w") as f_out:
...
def log_coverage():
    ...
    for proc in mwmon.data.procs:
        if (
            mwmon.coverage_procs is None
            or proc.proc_name in mwmon.coverage_procs
        ) and os.path.isfile(mwmon.coverage_log_name + (".%x" % proc.pid)):
            try:
                current_vad = None
                with open(
                        mwmon.coverage_text_name + (".%x" % proc.pid), "w") as f:
                    with open(
                        mwmon.coverage_log_name + (".%x" % proc.pid), "rb") as f_in:
...
def log_calls():
    ...
    f_out = open(f"{mwmon.api_tracer_text_log_name}.ordered", "w")
    try:
        for proc in mwmon.data.procs:
            if mwmon.api_tracer_procs is None or proc.proc_name in mwmon.api_tracer_procs:
                f_out.write("Process (PID: %x) %s\n" %
